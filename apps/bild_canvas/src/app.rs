...
use crate::msg::{create_duplex_plugin, msg_channel::GlobalEventChannel};
use crate::{
    circuit::CircuitPlugin,
    msg::{BildInMsg, BildOutMsg},
    view::layout_3d::Layout3dViewPlugin,
};
...
pub static GLOBAL_EVENT_CHANNEL: Lazy<GlobalEventChannel<BildInMsg, BildOutMsg>> =
...
impl Plugin for AppPlugin {
    fn build(&self, app: &mut App) {
        ...
        let (duplex_plugin, js_rx, js_tx) = create_duplex_plugin::<BildInMsg, BildOutMsg>();
        ...
        app.add_systems(
            Update,
            (
                BildInMsg::handle.run_if(on_message::<BildInMsg>),
                BildOutMsg::handle.run_if(on_message::<BildOutMsg>),
            ),
        );
