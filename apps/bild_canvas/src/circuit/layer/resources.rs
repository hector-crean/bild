...
use bevy::{color::Color, ecs::{component::Component, entity::Entity, resource::Resource}, platform::collections::HashSet};
use grid_2d::Grid2D;
...
where K: std::hash::Hash + Eq + Copy,
    ...
    pub fn insert(&mut self, k: K, v: V) {
        ...
        if let Some(old_v) = self.forward.get(&k).copied() {
            if let Some(set) = self.reverse.get_mut(&old_v) { set.remove(&k); }
        }
    ...
    pub fn remove_key(&mut self, k: &K) -> Option<V> {
        self.forward.remove(k).inspect(|v| {
            if let Some(set) = self.reverse.get_mut(v) { set.remove(k); }
        })
    }
...
#[derive(Resource)]
pub struct LayerStack<T: Grid2D + Default + Component> {
    pub layers: HashMap<LayerId, Layer<T>>,
    entity_layer_map: BiMap<Entity, LayerId>,
}

impl<T: Grid2D + Default + Component> Default for LayerStack<T> {
    fn default() -> Self {
        Self {
            layers: HashMap::new(),
...
pub struct LayerRef<'a, T: Grid2D + Default + Component> {
    layer: &'a mut Layer<T>,
}

impl<'a, T: Grid2D + Default + Component> LayerRef<'a, T> {
...
impl<T: Grid2D + Default + Component> LayerStack<T> {
