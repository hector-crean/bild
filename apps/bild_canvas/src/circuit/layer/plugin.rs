        ...
        + Component,
{
    fn build(&self, app: &mut App) {
        app.register_type::<super::types::LayerId>()
            ...
            .add_message::<LayerCommand>()
            .add_message::<LayerEvent>()
            .add_systems(
                Update,
                (
                    Self::apply_layer_commands.run_if(on_message::<LayerCommand>),
                    Self::on_layer_added,
...
impl<T> Default for LayerManagerPlugin<T>
where
    T: Grid2D
        + Send
        + Sync
        ...
        + Component,
{
    ...
    fn on_layer_changed(
        ...
        for (entity, on_layer) in q_changed.iter() {
            if let Some(mut layer_ref) = index.layer_mut(on_layer.0) {
                layer_ref.entities_mut().insert(entity);
            }
        }
    }

    fn on_layer_removed(index: ResMut<LayerStack<T>>, mut removed: RemovedComponents<OnLayer>) {
    ...
    fn apply_layer_commands(
        mut cmd_rdr: MessageReader<LayerCommand>,
        mut stack: ResMut<LayerStack<T>>,
        mut evt_wtr: MessageWriter<LayerEvent>,
    ) {
