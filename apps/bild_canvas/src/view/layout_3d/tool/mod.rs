...
use ts_rs::TS;
use ui::widget_2d::toolbar::ToolbarState;
use ui::widget_3d::radial_menu::RadialItemData;

use crate::{circuit::{CircuitPlugin, part::Part}, msg::BildOutMsg, view::layout_3d::CameraSettingsImpl};
...
impl LayerLike for GroundPlane {}
            ...
            .add_systems(
                ...
                (
                    handle_tool_state_transition
                        .run_if(on_message::<StateTransitionEvent<ToolState>>),
                ),
            );
    }
}
...
fn handle_tool_state_transition(
    ...
    mut rdr: MessageReader<StateTransitionEvent<ToolState>>,
    mut crayon_out: MessageWriter<BildOutMsg>,
) {
    info!("Tool state changed to: {:?}", tool_state.get());
    for StateTransitionEvent { exited, entered } in rdr.read() {
        match (exited, entered) {
            (_, Some(to)) => {
                crayon_out.write(BildOutMsg::ToolChanged(*to));
            }
            _ => {
