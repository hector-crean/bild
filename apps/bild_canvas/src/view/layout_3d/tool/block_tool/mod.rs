...
use ui::widget_3d::radial_menu::{
    CloseRadialMenu, OpenRadialMenu, RadialMenuPosition, RadialMenuSelection
};
use strum::IntoEnumIterator;
use crate::circuit::layer::LayerLike;
...
pub struct BlockToolPlugin<T: Block3DLike + ToRadialItem + IntoEnumIterator + Send + Sync + 'static, L: LayerLike> {
    phantom_block: PhantomData<T>,
    phantom_layer: PhantomData<L>,
}

impl<T: Block3DLike + ToRadialItem + IntoEnumIterator + Send + Sync + 'static + Default, L: LayerLike> Plugin for BlockToolPlugin<T, L> {
    fn build(&self, app: &mut App) {
        app
            .init_state::<BlockToolState>()
            .add_message::<BlockToolEvent<T>>()
...
impl<T: Block3DLike + ToRadialItem + IntoEnumIterator + Send + Sync + 'static + Default, L: LayerLike> BlockToolPlugin<T, L> {
    fn handle_state_transition(
        mut state_reader: MessageReader<StateTransitionEvent<BlockToolState>>,
    ) {
    ...
    fn handle_radial_menu_selection(
        mut selection_events: MessageReader<RadialMenuSelection>,
        mut next_state: ResMut<NextState<BlockToolState>>,
        commands: Commands,
    ...
    fn handle_confirmation(
        ...
        mut block_events: MessageWriter<BlockToolEvent<T>>,
    ) {
    ...
    fn open_radial_menu(
        mut event_wtr: MessageWriter<OpenRadialMenu>,
    ...
    fn close_radial_menu(mut event_wtr: MessageWriter<CloseRadialMenu>) {
