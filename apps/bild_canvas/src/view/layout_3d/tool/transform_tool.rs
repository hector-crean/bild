...
#[derive(PartialEq, Eq, Hash, Clone, Copy, Debug, Reflect, EnumIter, Event, Message)]
...
#[derive(Default)]
pub struct TransformToolPlugin<S: CameraSettings, L: LayerLike> {
    phantom_camera: PhantomData<S>,
    phantom_layer: PhantomData<L>,
}

impl<S: CameraSettings, L: LayerLike> TransformToolPlugin<S, L> {
    pub fn setup(
    ...
    fn handle_transform_state_transition(
        ...
        mut state_events: MessageReader<StateTransitionEvent<TransformToolState>>,
    ) {
    ...
    fn consume_transform_event(
        mut event_reader: MessageReader<TransformEvent>,
...
impl<S: CameraSettings, L: LayerLike> Plugin for TransformToolPlugin<S, L> {
